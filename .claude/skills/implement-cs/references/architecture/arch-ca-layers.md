...
- **Observable ベース**: 未捕捉例外は `IUncaughtExceptionHandler` の Observable で通知
- **Result 型は不使用**
...
`UncaughtExceptionHandler` が Unity ログシステムから `LogType.Exception` をフィルタし、`HecError` を生成して `Observable<HecError>` で通知する。Singleton として DI 登録され、Presentation 層のエラーダイアログ表示に至る。

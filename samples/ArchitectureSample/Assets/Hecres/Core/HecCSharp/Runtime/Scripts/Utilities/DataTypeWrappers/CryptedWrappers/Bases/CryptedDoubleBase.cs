...
namespace Hecres.Core.HecCSharp.Utilities.DataTypeWrappers.CryptedWrappers.Bases
{
    ...
    /// 暗号化対応doubleの型クラスの基底
    /// </summary>
    public abstract class CryptedDoubleBase<T> : ICryptedDoubleValue, IComparable<T>, IEquatable<T>, IFormattable
        where T : CryptedDoubleBase<T>
    {
        /// <summary>
        /// 等価判定の許容誤差
        /// </summary>
        public virtual double EqualsTolerance { get; } = 0.00001f;

        /// <summary>
        /// 復号後の値
        /// </summary>
        public double Value => IsEncrypted() ? HecEncryptor.DecryptDouble(encryptedValue, seed) : plainValue;

        private readonly double plainValue;
        ...
        protected CryptedDoubleBase(double plainValue, bool useEncryption, Func<double, bool> isValidFunc, Func<double, double> processPlainValueFunc)
        {
        ...
        /// <param name="target">比較対象のオブジェクト</param>
        /// <returns>true: 等しい / false: 等しくない</returns>
        public bool Equals(T target) => target != null && Math.Abs(Value - target.Value) < EqualsTolerance;
        ...
        /// 等価演算子
        /// </summary>
        public static bool operator ==(CryptedDoubleBase<T> a, CryptedDoubleBase<T> b) => Equals(a, b);
        ...
        /// 小なり演算子
        /// </summary>
        public static bool operator <(CryptedDoubleBase<T> a, CryptedDoubleBase<T> b) => a.Value < b.Value;
